...
export function Import() {
  ...
  const handleImport = async () => {
    ...
    for (let i = inputMethod === 'csv' ? 1 : 0; i < rows.length; i++) {
      const cells = rows[i].split(/[,;]/).map(cell => cell.trim().replace(/^"|"$/g, '')); // Remove quotes if present
      const id = cells[idIndex];

      if (id) {
        movieIds.push(id);
        watchedStatus.push(watchedIndex !== -1 ? cells[watchedIndex] === '1' : false);
        favouriteStatus.push(favouriteIndex !== -1 ? cells[favouriteIndex] === '1' : false);
      }
    }
    ...
    const existingWatchlistIds = new Set(user.watchlist.map(item => item.movie.id.toString()));
  ...
  return (
    <div className="text-neutral-500 text-sm">
      <p className="mb-4">
        Currently, it is only possible to import Movies using The Movie Database (TMDB) IDs. TV Shows and import from IMDb or Trakt will be available in the future.
      </p>

      <div className="flex gap-1 p-1 bg-neutral-900 rounded-xl mb-2">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => {
            ...
            className={`${
              inputMethod === tab.id ? "text-black font-semibold" : "--md:hover:opacity-50"
      ...
      <div className={`px-4 pt-3 rounded-xl bg-neutral-900`}>
        {inputMethod === 'csv' ? (
          <>
            Your file should contain the following columns:
            <ul className="list-disc [&>li]:ml-4 mb-1.5">
              <li>Movie ID</li>
            ...
            <label 
              htmlFor="file" 
              className={`relative block w-full mt-3 py-3 text-center border-t border-neutral-800 ${fileName ? 'flex gap-2 ' : ''}`}
            >
              ...
              {fileName && ( 
                <span className="truncate">
                  <File className="inline-block size-[1em] -mt-[0.2em] mr-0.5 flex-shrink-0" />
                  {fileName}
                </span>
              )}
            </label>
          ...
          <textarea
            ...
            className={`block w-full min-h-32 outline-none resize-y placeholder-neutral-500 bg-neutral-900 pb-3`}
          />
        )}
        {(inputMethod === 'textarea' && inputValue) || (inputMethod === 'csv' && fileName) ? (
          <button onClick={handleImport} className="relative w-full py-3 text-blue border-t border-neutral-800">
            {loading ? ( 
              <>
        ...
        <>
          ...
          {message === 'success' ? (
            <span className="flex items-center gap-2">
              <Check className="size-4 text-green border border-green rounded-full p-0.5" />
