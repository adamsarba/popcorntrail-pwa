...
export function BottomBar({ onSearchClick, onAddListClick }: BottomBarProps) {
  ...
  return (
    <div className={`${isPWA ? "h-[5.25rem]" : "h-auto"} fixed bottom-0 w-full backdrop-blur-xl bg-black/50 px-4 py-3 flex justify-between items-start gap-4`}>
      <button 
      ...
      <button onClick={onAddListClick} className="text-accent">Add List</button>
    </div>
  );
}